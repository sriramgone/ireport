...
<?php
		...
		$victimname = filter_input(INPUT_POST,'victimname');

    $sql = "SELECT accidentid,victim_name,hospital_name,date,time FROM accident,hospital
          WHERE hospitals = hospital_id AND victim_name LIKE '$victimname%'
    ...
    if ($result->num_rows > 0) {
      ...
      echo "<div id='card'><br></br><p>ENTER REPORTED NAME</p><form action='verifieduser_home.php' method='post'><button type='submit' id='done'>Done</button><br></br></form></div>";
    }
